...
| 版本 | 日期 | 核心改动 |
|---|---|---|
| V6.0 | 2026-09-09 | 生产系统完成定义（结构对齐·非词库扩充）：①版本体系统一（消除 V6.0/5.31/5.32-5.50 四套表述混用）②历史归档结构确认（V1.0-V5.6 完整归档已就位）③实施包 V5.32→V6.0 一次性蓝图原样存档为「附录·执行层蓝图」（规划非规则·逐段验收转正·与正文冲突以正文为准）④正文区保持 V5.32 已验收六项修复为当前生效规则 |
| V5.32 | 2026-09-09 | 规则消歧二轮·旧规则隔离收尾（评审驱动·六件）：①路由「十四/字幕」错位修复（视频正式=VID-1.4 14步详装逐项生成·字幕对白文字卡=后期层独立输出不入 Prompt 主体）②V4.2.2 旧生产链接入 DIR-QC（VIDEO-QC → CONTINUITY → DIR-QC → EDIT/REWORK）③案例库【净化状态】字段+点名案例语义压缩（奶油散景/ARRI 案例锚点 ≤2）④ADP-1.0 加 MASTER→平台字段映射表（输入→输出转换规则）⑤5s 旧案例统一【历史兼容案例/5s特例】+时长自适应换算规则⑥四感默认=基调参数不计入锚点配额（规则 9 定义） |
| V5.31 | 2026-09-09 | 规则消歧·旧规则彻底隔离·平台适配层（评审驱动·六件事）：①版本表收敛（顶部只留 5 条·V5.28 补录·V5.24 以下入修订日志）②14 步章节隔离（仅限 VID-1.4 视频正式结构·图片正式交付不得调用）③规则 9 追加案例库不豁免条款（规则与案例冲突以规则为准）④光圈逻辑修正（示例值仅供参考·不构成景别-光圈绑定）⑤生产流程同步 DIR-QC（VIDEO-QC → CONTINUITY → DIR-QC → EDIT）⑥新增 ADP-1.0 平台适配层（MASTER PROMPT → 平台适配器 → 平台 Prompt·GPT Image-2/H3/Seedance/即梦/可灵） |
| V5.30 | 2026-09-09 | 规则清洗·冲突消除·唯一规范·AI 执行优先级（评审驱动·六件事）：①版本体系双轨制（文档版本 WCF V5.30 / 模块版本 PM-IMG-VID-CONT-QC-LIB 各自演进；历史段统一标"仅供追溯"）②规则优先级+Prompt 权限等级（LOCKED/FIXED/SELECT/GENERATIVE/OPTIONAL 五级+优先级链+默认权限+违规即生产事故）③H3 唯一主规范（V5.11 五模式+三字段=结构层唯一有效；V4.2.7 八层独有内容并入内容层清单后归档；V4.2.8 5秒模板由时长自适应节拍模板取代）④输出结构分家（视频正式=14 步/图片正式=9 步/快速=视频 7 段·图片 5 字段·〇章规则 6 改写）⑤时长自适应节拍模板（0-15%/15-60%/60-85%/85-100%·4-30s 速算表）⑥导演级 QC（DIR01-06·QC 三层金字塔·镜头卡新增"镜头目的"字段） |
| V5.29 | 2026-09-09 | 六章案例库补「CASE 21 雨夜双人·仙侠结印对敌」（H3 图生视频 8s 完整案例·用户实测动作设计）：①招式链（女子独钻印→内缚印→剑诀外放三式递进 × 男子含光抱月起势→沉腕抬剑→弓步剑平+剑指辅势）②神态微动作清单（睫毛低垂再抬/眼瞳微缩/目光如刃/嘴角锐利弧度/喉结滚动/持剑指节微收/雨珠自眉骨滑落等）③法阵四段动态（低频脉动→内圈点亮+顺时针旋转→卦象浮现+加速外扩→全速最亮骤止）④呼吸纪律（全片呼吸动作只写一次，联动剑光/法阵）⑤音乐音效三段递进（静→单音逐颗→琶音鼓点→顶点收束）⑥8s 三字段完整提示词可直接注入 H3 |

### 📒 修订日志（V5.28 及以下·供追溯·顶部版本表只留最近 5 条）

| 版本 | 日期 | 核心改动 |
|---|---|---|
| V5.28 | 2026-09-09 | 校园/教师题材入库（补录·正文实存）：①教师角色卡 T01 ②校园教室场景库 S39-S44（6 张实测图提炼·教室文字保护铁律） |
| V5.27 | 2026-09-09 | 七大扩充类目入库（用户实测·现代/运动/校园/仙侠/舞蹈/兵器/神态）：①场景库补「S32-S38 现代场景库」（咖啡馆/地铁车厢/写字楼/便利店/天台/出租屋/街角夜市·场景六件套）②动作库补「体育运动 A100-A108」（跑步/篮球运球跳投/自由泳蛙泳/瑜伽下犬式猫牛式/骑行/足球盘带）③动作库补「校园动作 A109-A115」（课堂记笔记/举手发言/图书馆自习/课间喧闹/走廊背书/操场集合/考前复习）④结印手势库后补「仙侠法器手势库」（掐诀/御剑/控器/施法四类+法器手势写作公式）⑤舞蹈动作后补「舞蹈种类库」（惊鸿/霓裳/胡旋/胡腾/柘枝/盘鼓/绿腰/剑器/长袖/折腰·10 舞种点题表）⑥打斗拆招库后补「兵器对打句式库」（剑对剑/刀对剑/枪对剑/奇门对兵+节奏公式）⑦古风神态词汇库后补「通用神态库」（12 情绪→生理落点速查表+通用神态公式） |
| V5.26 | 2026-09-09 | 现代/通用动作库入库（用户实测·不限于古风）：动作库补「现代通用动作库 A87-A99」（通勤刷手机/地铁阅读/敲键盘办公/咖啡杯沿水珠/跑步锻炼/晨起摸手机/厨房备餐/骑车通勤/行走数砖/社交加友/捋发侧耳/瘫坐刷屏/健身房拉伸·13 条）+「现代动作写作公式」（道具+动作链+环境互动+微神态）；现代题材（都市/职场/校园/咖啡）直接调用本库 |
| V5.25 | 2026-09-09 | 动作库/武术库二轮扩充（用户实测·日常+舞蹈+打斗+神态）：①动作库补「日常劳作动作 A25-A30」（浣衣/纺线/捣练/推磨/劈柴/挑水·环节拆解写法）②动作库补「舞蹈动作 A31-A36」（水袖抛挽/回旋/剑舞起势/劈撩/折腰软舞/飞天·发力点→轨迹→收势写法）③微表情表演库后补「古风神态词汇库」（眉眼/目光/唇齿/笑态/面容/气质六组选词+神态写作公式）④兵器库⑧后补「打斗实战拆招库」（回合写法·征兆→拆解→落点+拆招句式库+身法步法+打斗写作公式）⑤兵器库补「⑧ 武术基础·步型拳掌腿法·招式库」（步型五式弓马虚仆歇+拳掌腿法24招+太极名式+武侠招式名库+武打动词+打斗写作公式） |
| V5.24 | 2026-09-09 | 实战手印库+剑术库入库（用户实测·九字真言/道教指诀+传统剑术）：①动作库补「结印手势库 A16-A24」（九字真言：独钻印/大冲虚宝印/外狮子印/内缚印/智拳印；道教指诀：剑诀/子午诀/三清指/清净诀·含掐位指法·搜「结印」「手印」「指诀」调出）②兵器库专章补「⑦ 剑术招式与持剑法」（持剑四法满把/钳把/刁把/压把+剑指规范+剑法十六字刺劈点撩挑崩截斩抹削云挂架压绞扫+起势式·含光抱月+弓步直刺/撩剑/点剑/提膝劈剑+剑招写作公式·搜「剑招」「剑术」「持剑」调出）③结印时间线写法（起手→内聚→外放·能量随呼吸脉动） |
| V5.23 | 2026-09-09 | 新增「建筑风格库专章」（3.1 场景描写区）：①建筑类型 8 类（宫殿/园林/宅院/寺庙/城防/市井/高塔/桥梁）②屋顶形制 7 式（庑殿/歇山/悬山/硬山/攒尖/卷棚/重檐）+脊饰③结构构件（斗拱/藻井/台基/雀替/窗棂等）④材料质感（青砖黛瓦/琉璃瓦/马头墙等）⑤建筑氛围光效句 6 条⑥建筑×题材适配表 |
| V5.22 | 2026-09-09 | 兵器库专章扩展：⑤兵器×服饰造型妆造配套表（剑客/重剑/刀客/枪将/弓手/刺客/奇门/棍僧 8 系·服装+发型妆造+配饰+气质）⑥武器光效大全（8 系 20+ 条光效互动句） |
| V5.21 | 2026-09-09 | 新增「兵器库专章」（道具雅词库前）：①制式样式长短 7 大类（剑/刀/枪矛/长兵重兵/钩叉拐/弓弩/奇门·点题雅词）②暗器 4 类（飞针/飞刀镖/袖箭机簧/绳爪链）③纹饰纹理质感（剑身刀身部件/锻造纹理/装饰纹样/工艺质感）④兵器光效互动句 7 条 |
| V5.20 | 2026-09-09 | 吸收武侠变体四例+暗黑风格：①3.5.18 补 CASE 13-16（提灯侠女/暗夜金纹/伸手挽留/竹林策马·双语+要点）②风格基调补 V7「暗黑武侠·黑金冷峻」③双人互动配方补「张力叙事位」（背身挽留/伸手未及/背道相错）④词补全：仙侠法器·发光系（灵剑/法剑/荧光符箓）、夜雾 |
| V5.19 | 2026-09-09 | 吸收抖音@自热包「雾中竹林骑马」武侠实测：①3.5.18 案例库补 CASE 12（MJ系英文原版+中文翻译+七段拆解·正向锁定不看镜头/无面部特写）②画质净化段后补「次世代游戏 CG 画质锚点」（PBR材质/SSS次表面散射/超清晰纹理/弥散光晕/氛围雾等词系列补全） |
| V5.18 | 2026-09-09 | 吸收抖音「中式美学·第十八期」GPT-Image 九图实测：①六章新增「道具路径构图法」（巨袖/长道具/花枝引导视线·先定落点再让道具带路）②3.5.18 案例库补全 11 个 CASE（CASE01-02 GPT-Image 双语范本 + CASE03-09 其余七图中文版 + CASE10-11 MJ 田园风双语）③十七章负面词库补 GPT-Image 数字人像负面范式（白棚光/布料漂白/哑光插画感/棕褐橄榄色偏/界面文字水印） |
| V5.17 | 2026-09-09 | 吸收抖音光影方法论（意识上载·13万播放教程）：①新增「镜头光学质感速查表」（六章·色差/运动模糊/畸变/景深/散景·三档参数化+英文关键词）②新增「光影五步写作法」（二章·先拒绝→总体要求→打光方法→空气介质→约束+信息图先行）③人物光影×情绪对照表补 3 行+平均光反面警示 ④题材表补国风古韵 5 行 |
...
| 生产管理系统（任务单/镜头卡/QC/成本/版本/验收） | PM-1.3（原"生产管理系统 V5.2"） |
| 双引擎生产（GPT Image-2 定形 × MiniMax H3 定动） | IMG-1.2 × VID-1.4（原"双引擎 V5.3"） |
| 后期字幕文案 | 十四 |
| 结印手势（九字真言/道教指诀·独钻印/剑诀/子午诀等） | 动作库 A16-A24 |
| 日常劳作动作（浣衣/纺线/捣练/推磨/劈柴/挑水） | 动作库 A25-A30 |
| 舞蹈动作（水袖/剑舞/飞天/折腰软舞） | 动作库 A31-A36 |
| 现代通用动作（通勤/办公/咖啡/健身/玩手机） | 动作库 A87-A99 |
| 现代场景（咖啡馆/地铁/办公室/天台/夜市） | 场景库 S32-S38 |
| 体育运动（跑步/篮球/游泳/瑜伽/骑行） | 动作库 A100-A108 |
| 校园动作（课堂/自习/课间/图书馆/操场） | 动作库 A109-A115 |
| 仙侠法器手势（掐诀/御剑/控器/施法） | 仙侠法器手势库（结印手势库后） |
| 舞蹈种类（惊鸿/胡旋/盘鼓/绿腰/霓裳） | 舞蹈种类库（舞蹈动作后） |
| 兵器对打句式（剑/刀/枪/奇门双人交手） | 兵器对打句式库（打斗拆招库后） |
| 通用神态（12 情绪→生理落点速查） | 通用神态库（古风神态词汇库后） |
| 古风神态词汇（眉眼/目光/唇齿/笑态选词） | 古风神态词汇库（微表情表演库后） |
| 剑招/持剑法（起势/弓步直刺/撩剑/剑指规范） | 兵器库专章 ⑦ |
| 武术步型/拳掌腿法/太极名式/招式名 | 兵器库 ⑧ |
| 打斗实战拆招（回合写法·征兆→拆解→落点） | 兵器库 ⑧·打斗实战拆招库 |
| 运营避坑 | 十五 |
| 不同工具怎么喂 | 十六 |
| 不要什么 / 常见翻车点 | 十七 / 十九 |
| 雨夜双人·仙侠结印对敌（8s 完整 H3 案例·招式链/法阵动态/神态微动作/呼吸纪律/音乐音效） | 六·CASE 21（V5.29） |
| 声音 / BGM / 音效 | 十八 |
| Prompt 权限等级（LOCKED/FIXED/SELECT/GENERATIVE/OPTIONAL） | 〇·规则 8 |
| 风格锚点配额（反电影感堆词硬规则） | 〇·规则 9 |
| 导演级 QC（DIR01-06·镜头是否拍对） | PM-1.3·六（视频 QC 后） |
| 时长自适应节拍模板（4-30s 速算表） | 七章 ★ H3 唯一主规范节 |
| 版本体系双轨制（文档版/模块版） | 顶部·版本记录后 |
...
> 6. **输出结构路由（V5.30 起·视频/图片彻底分家）**：视频正式交付 → 「七、14 步详装结构」逐项输出（第 3 步「场景与元素」——人群/动物/植物/器物陈设逐一点名；第 13 步「构图角度」改为「运镜」）；图片正式交付 → 「〇、图片 9 步精装结构」逐项输出（对标视频 14 步，去运镜字段）；快速出片 → 视频用 7 段总装公式、图片用 5 字段压缩版。**人物七件套/场景元素等"描写层"两结构共用**。路由总表：视频正式=14 步｜图片正式=9 步｜视频快速=7 段｜图片快速=5 字段
> 7. **人物七件套（写人必带）**：第 2 步「主体」写人物时，默认按七件套写全——① 年龄锚点 ② 面部（脸型/眼/眉/鼻/唇）③ 妆发（发型/妆容/首饰）④ 身材（身形/曲线/体态）⑤ 服装（三段式分层）⑥ 动作·道具·神态 ⑦ 气质词；除非用户明确要极简描述，否则缺层要补
> 8. **Prompt 权限等级（V5.30 总闸门·AI 不得越权发挥）**：用户口述或项目锁定的元素按五级权限执行——**LOCKED**（用户明确指定，AI 不得修改，如"穿宋制圆领袍，手持油纸伞"）、**FIXED**（项目已确定，不得主动替换，如系列主角面容/服装）、**SELECT**（允许从素材库 LIB 选择，如配饰）、**GENERATIVE**（允许自由补充，如氛围/光影细节）、**OPTIONAL**（可有可无，不得影响主体，如远景飞鸟）。优先级链：**用户显式指令 > LOCKED > FIXED > 文档硬规则 > SELECT > GENERATIVE > OPTIONAL**。默认值：用户口述的核心元素（身份/服装/道具）默认 LOCKED，氛围光影默认 SELECT。**AI 修改 LOCKED/FIXED 内容 = 生产事故**，按返工单记录并回退
> 9. **风格锚点配额（V5.30 硬规则·全文档生效·与 V5.7 反模板化闭环）**：每条 prompt 的风格锚点词（电影感/电影级/cinematic/film grain/大片感/史诗感 等）**最多 2 个**；写作顺序固定「镜头判断 → 空间关系 → 主体动作 → 光源逻辑 → 材质 → 镜头参数 → 最后才允许 1-2 个风格锚点收尾」。图片风格选单 P1-P5 各算 1 个合法锚点。违者回退重写。**案例库不得作为风格锚点数量的豁免项（V5.31 追加）**——案例中出现的风格词同样受本规则约束，规则与案例冲突时以规则为准。**四感边界（V5.32 定义）**：四感默认（柔和·唯美·细腻·电影化）属"基调参数"，整体不计入风格锚点配额；P1-P5 选单、cinematic、film grain、大片感、史诗感等显式风格锚点计入配额；"电影化"仅在作为四感默认整体输出时豁免，单独作风格词使用时计入配额

### 根据用户回答，AI 执行对应路径

| 用户回答 | 执行路径（一条龙） |
|---|---|
| 视频 + 任何题材 | 视频正式交付 → 按 VID-1.4「14 步详装结构」（七章）逐项生成；**字幕/对白/文字卡属后期层，独立输出（文案见「十四」），不进入 14 步主体 Prompt**；题材取值查「十三」 |
| 图片 + 古风/现代题材 | 用下方「图片生成入口风格选单」选基调 + 单画面描述 |
| 图片 + 科幻/其他非常规题材 | 用图片选单选基调 + 用户描述场景 |
| 两者都要 | 先出图片作参考帧/海报 → 再用图片首帧跑图生视频（分两步，先图片后视频） |
| 直接生产 / 收到 PROJECT 任务单 | 走「PM-1.3 → IMG-1.2 × VID-1.4」：任务单 → C06 素材调用 → 镜头卡 → GPT Image-2 出图（母版）→ IMAGE-QC → 首帧锁定 → MiniMax H3 生视频 → VIDEO-QC → C05 连续性 QC → 成片验收；素材锁定 CHAR/OBJ/SET；双引擎职责：图片定形 / 视频定动 |
...
### 图片 9 步精装结构（图片正式交付默认结构·对标视频 14 步·路由见〇章规则 6）
...
8. **质感+画质**：奶油散景 + 轻微胶片颗粒（风格锚点 ≤2）；浅景深（技术参数）；绢帛质感（材质）【净化状态 PASS】
...
### 次世代游戏 CG 画质锚点（V5.19 入库·武侠/奇幻/游戏风出片用·垫在场景人物之后）

> 适用：游戏 CG 风、武侠/奇幻题材、要"写实渲染感"不要"照片感"的片子。整段或挑词接入。

**整段**：次世代游戏 CG 渲染，超写实 PBR 材质，超清晰纹理，丰富的 SSS 次表面散射，逼真的布料飘动与毛发细节，弥散光晕与氛围雾气，精致的中式美学，高细节。

**关键词库（词系列补全）**：
- 渲染感：次世代游戏CG, 超写实渲染, 电影级渲染, 高细节
- 材质系：PBR材质, 超写实PBR, 物理正确材质, 超清晰纹理, 丰富的材质层次
- 皮肤/布料系：SSS次表面散射, 通透皮肤质感, 真实布料飘动, 真实毛发纤维
- 氛围系：弥散光晕, 氛围雾气, 大气透视, 尘埃微粒
...
- **远景首帧（定场用）**：电影剧照感。暮色朱漆大门全景，媒人捧雁立于阶前，人物占画面 10%，头顶留白 30%。侧逆暖光，琥珀色温。焦段参考 24-50mm，光圈按景深目的（环境叙事建议 f/4-f/5.6）。
- **中景首帧（叙事用）**：电影剧照感。媒人捧雁立于阶前，人物占画面 40%，头顶留白 25%。大雁在前景清晰，人物后方虚化。侧逆暖光。焦段参考 50-85mm，光圈按景深目的（主体-前景关系建议 f/2-f/2.8）。
- **近景首帧（情绪用）**：电影剧照感。媒人双手捧雁，雁笼红绸结特写，人物半身入画，头顶留白 20%。侧逆暖光，金尘浮动。焦段参考 85-135mm，光圈按景深目的（情绪特写建议 f/1.8-f/2.8）。
> **焦段规则（V5.30 修正·替换旧"三景别锁 85mm"写法）**：焦段是构图工具而非固定参数——远景 24-50mm（空间拉伸/环境叙事）、中景 50-85mm、近景 85-135mm（空间压缩/主体隔离），均为**参考区间不是锁死值**；光圈由景深目的决定，不与景别强绑定（主体隔离/情绪特写 → 大光圈浅景深；环境叙事/群体互动 → 小光圈深景深）。**同一镜头组保持相近焦段与机位高度**（一致性优先，呼应"母版复用"），焦段自由但组内不跳变。

> 用法：同一场景按"远景/中景/近景"各出一张，图生视频时再挑最接得住下一镜的那张。
> **光圈选择逻辑（V5.31 修正·示例值仅供参考，不构成景别—光圈绑定规则）**：光圈由景深目的决定——需要前后景都清晰（环境叙事/群体互动）用小光圈；需要背景虚化突出主体（情绪特写）用大光圈。起点参考：远景 f/4-f/5.6、中景 f/2-f/2.8、近景 f/1.8-f/2.8；**不得机械按景别套光圈**；同一镜头组保持相近光圈与焦段（一致性优先，呼应"母版复用"）。
...
**GPT-Image 实测 CASE 01：花团框景与蓝色牡丹大袖**（中式美学·第十八期·16:9 ·参数 --ar 16:9 --s 100 --c 0 --v 8.2）

> 中文（可直接喂豆包/即梦）：
16:9 横幅中景正面肖像，女子略偏画面右侧，从完整发冠一直呈现到腰部以下。面部落在右上方交点附近，一只手臂向左侧画外抬起；薄雾水青色刺绣巨袖从左下方进入，横穿画面下方三分之二区域后回到躯干，以一朵超大浅金牡丹作为最醒目的前景图案。左侧三分区与最右边缘以柔焦奶油色花朵构成横向花窗，但不能遮住双眼。她目光平稳直视，珊瑚色嘴唇轻合，额间有小型红色花钿；亮泽黑发盘成高髻，配珍珠金色植物簪和长珍珠流苏。体积化数字肖像表现，面部渐变连续、反射连贯、刺绣清晰并具镜头景深。象牙白内层真丝与蓝色外层织锦上有凸起的香槟金、浅珊瑚和低饱和青绿色细节，并与透明欧根纱清楚分层。左上方暖阳在额头、面颊和颈部投下选择性的枝叶影；适度中性补光让面部和巨袖从烟熏卡其背景中分离。高光晕染只出现在巨型牡丹金线、珍珠和袖褶脊线上。

> 负面（画面反推·非原始生成参数）：避免巨型牡丹袖被裁切、花团遮住双眼、额饰缺失、手臂与袖子粘连、刺绣变成平面印花、白棚光、布料漂白、哑光插画感、棕褐或橄榄色偏、界面文字与水印。

> 结构拆解（六要素）：镜头=16:9横幅中景正面肖像｜场景=柔焦奶油色花窗+烟熏卡其背景｜人物=高髻+花钿+珍珠流苏｜动作=单臂抬起引袖入画｜光色=左上暖阳选择性枝叶影+中性补光｜避错=巨袖不裁切、花不遮眼

**GPT-Image 实测 CASE 02：牡丹花冠、侧望与透明披帛**（中式美学·第十八期·16:9 ·参数 --ar 16:9 --s 100 --c 0 --v 8.2）

> 中文：
16:9横幅胸部以上三分之二侧身肖像，女子固定在右侧三分区，面部转向画面左侧，目光穿过宽阔的暖灰褐负空间。完整象牙白与淡粉牡丹花冠横向铺在右上方，不能被裁切；一条半透明珍珠质感欧根纱披帛从肩侧起始，横扫画面下方向最左端延伸，第二层薄纱在身后柔和扬起，一只手在右下方完整可见并握着珠宝饰件。她神情遥远而沉思，珊瑚色嘴唇轻合；亮泽黑发盘成宫髻，面侧留有弯曲散发，配银蓝色植物金工、珍珠、水晶和垂坠耳饰。体积化数字肖像表现。薄雾蓝织锦上有凸起淡粉牡丹、银色叶片和克制香槟金线，内层胸衣为暖珊瑚色。左上方中等柔度暖阳在额头和近侧眼睛投下一道清楚弧形影；柔和中性补光保留面部立体感。左侧留白比面部更暗，光晕只落在欧根纱脊线、水晶和金属刺绣上。

> 负面：避免牡丹花冠被裁切、侧望改成直视、披帛起止方向混乱、手部缺失或珠宝粘连、弧形影遮黑双眼、左侧留白消失、白棚光、布料漂白、哑光插画感、棕褐或橄榄色偏、界面文字与水印。

> 结构拆解：镜头=16:9横幅胸部以上三分之二侧身｜场景=暖灰褐负空间+左侧留白｜人物=宫髻+银蓝金工+珍珠水晶｜动作=披帛横扫下方向最左延伸｜光色=左上暖阳弧形影+中性补光｜避错=花冠不裁切、披帛方向不乱、左侧留白保留

**GPT-Image 实测 CASE 03：宫室横笛**（中式美学·第十八期·16:9 ·参数 --ar 16:9 --s 100 --c 0 --v 8.2）

> 中文：
16:9 横幅室内肖像，女子从头部到腰部占据右侧三分区；一支浅玉色横笛从左侧四分之一处起始，沿开放的画面中心斜向延伸至唇边。一只手在中左部托住横笛，另一只手在笛下舒展，两只手都完整可见。左半画面以分层景深呈现昏暗彩绘宫室天顶、悬挂灯笼与冷蓝色格窗；桃花虚化在左下前景，但不能遮住乐器。她目光平静微垂，珊瑚色嘴唇轻合，黑发盘成高而繁复的发髻，面侧留有松散发丝，配珊瑚花饰、金丝花叶和长坠耳饰。体积化数字肖像表现，面部渐变连续、反射逻辑真实并具镜头景深。低饱和青绿色织锦胸衣带香槟金牡丹浮纹，外围是桃色欧根纱大袖和象牙银色荷叶层。左上方午后暖光柔中带方向，保留下颌、横笛和衣褶下方的阴影；冷色窗光把人物从室内背景中分离。光晕只停留在金线、笛子金属配件和欧根纱折脊。

> 负面（画面反推·非原始生成参数）：避免重复横笛或多余乐器、笛身断裂弯曲、双手被裁切、手指融合、笛子穿过面部、桃花遮挡笛孔、白棚光、布料漂白、哑光插画感、棕褐或橄榄色偏、界面文字与水印。

> 结构拆解：镜头=16:9横幅室内肖像｜场景=宫室天顶+灯笼+冷蓝格窗｜人物=高髻+珊瑚花饰+长坠耳饰｜动作=一手托笛一手舒展，笛至唇边｜光色=左上暖光+冷窗光分离｜避错=乐器不重复、手不被裁、笛不穿脸

**GPT-Image 实测 CASE 04：回肩近景与花冠光环**（中式美学·第十八期·16:9 ·参数 --ar 16:9 --s 100 --c 0 --v 8.2）

> 中文：
16:9 横幅回肩肖像，女子固定在右侧三分区，从完整花冠呈现到胸部以下。身体背向画面左侧，面部越过近侧肩膀回望；纹理锋利的刺绣肩部占据右下前景，透明象牙白衣袖柔和地拖向画面下方中央。超大花冠从脑后弯曲并向画面中左部后退，形成层层虚化的光环；最左侧保留冷静的石板灰负空间与少量琥珀色散景。她以克制的侧目看向镜头，珊瑚色嘴唇轻合；亮泽黑发收成紧致低位宫髻，细碎发丝贴近面部，并配珍珠耳钉和纤细金坠。体积化数字肖像表现，面部渐变连续、反射关系连贯、织物细节清晰并具镜头景深。香槟象牙色缎面上有凸起的淡粉、珊瑚、低饱和青绿和浅蓝花朵；花冠由象牙花、透明蓝花瓣、金丝、珍珠和小珊瑚珠构成。左上方中等柔度暖光塑造眼睛、颧骨和肩部；面部与近侧刺绣保持清晰，后方花冠、衣袖和远景逐层柔化，光晕只在局部出现。

> 负面：避免花冠光环被裁断、回肩方向错误、双肩正对镜头、花冠压住眼睛、后景与面部同样锐利、刺绣糊成色块、白棚光、布料漂白、哑光插画感、棕褐或橄榄色偏、界面文字与水印。

> 结构拆解：镜头=16:9横幅回肩肖像｜场景=石板灰负空间+琥珀色散景｜人物=低位宫髻+珍珠耳钉+金坠｜动作=越肩回望，背向画面｜光色=左上暖光塑形+后方逐层柔化｜避错=花冠不裁断、回肩方向对、前后景锐度分层

**GPT-Image 实测 CASE 05：云海露肩回望**（中式美学·第十八期·16:9 ·参数 --ar 16:9 --s 100 --c 0 --v 8.2）

> 中文：
16:9 横幅越肩肖像，女子固定在右侧三分区，从头部呈现到腰部以下，左侧三分之二区域开放给有层次的蓝天和立体奶油白云。躯干转向画面左侧，面部回望观者；一侧裸肩形成干净斜线，长而微卷的黑色马尾沿右边缘垂落，一只巨大的象牙织锦袖从身体铺展并横越整个下方画面。她以克制的斜向目光看向镜头，珊瑚色嘴唇微启；亮泽宫廷发髻配齐凑金珍珠花饰和纤细水晶流苏。体积化数字肖像表现，面部渐变连续、反射连贯、珠饰清晰并具镜头景深。发光象牙真丝上有香槟金与冷银云纹浮雕，内层带淡桃色，褶皱具有雕塑感。左上方中等偏硬的暖日光与冷色天空补光平衡，保留面颊、下颌和肩部的立体塑形。远处云层更冷、更软，珍珠光晕只停留在金属线、饰品和丝绸脊线上。

> 负面：避免云海被裁空、人物居中挤满画面、露肩结构错误、马尾缺失、巨幅袖边被截断、肤色与天空混成一片、白棚光、布料漂白、哑光插画感、棕褐或橄榄色偏、界面文字与水印。

> 结构拆解：镜头=16:9横幅越肩肖像｜场景=蓝天+奶油白云分层｜人物=宫髻+金珍珠花饰+水晶流苏｜动作=裸肩斜线+巨袖横越画面｜光色=左上暖日光+冷天空补光｜避错=云海留足、马尾在、肤色与天空分离

**GPT-Image 实测 CASE 06：闭目花枝与金青披风**（中式美学·第十八期·16:9 ·参数 --ar 16:9 --s 100 --c 0 --v 8.2）

> 中文：
16:9 横幅背身肖像，女子位于右侧三分区，躯干朝向画面右侧，头部低垂向开放的左侧，双眼完全闭合。一枝缀满白花的枝条从最左前景开始，在发冠上方和后方横向通过，再延伸到画面右侧，成为贯穿蓝灰负空间的自然桥梁。左下还有额外白花虚化，青铜金与烟青色外袍从人物向右下和中央扩展。亮泽黑发盘成宽大平滑的高髻，以克制的多色珠簪和细长金耳坠装饰。体积化数字肖像表现，面部渐变连续、金属反射连贯、织物细节清晰并具镜头景深。外袍结合抛光青铜金真丝、烟青色拼片、稀疏珊瑚花纹、高象牙领和深朱红内层。右上方中等偏硬暖阳制造明亮金色闪光和清晰枝影；冷石板蓝环境补光保留面部与布料中间调。白花保持柔软立体，光晕只停留在金属布料和珠宝上。

> 负面：避免睁眼、人物转成正面直视、花枝切过双眼、枝条断裂漂浮、金色披风过曝成白片、衣料塑料感、白棚光、哑光插画感、棕褐或橄榄色偏、界面文字与水印。

> 结构拆解：镜头=16:9横幅背身肖像｜场景=蓝灰负空间+白花自然桥梁｜人物=高髻+多色珠簪+金耳坠｜动作=闭目低头，枝横穿发冠｜光色=右上暖阳金闪+冷石板蓝补光｜避错=双眼闭、枝不切眼、金披风不过曝

**GPT-Image 实测 CASE 07：发光蝴蝶与蓝花前景**（中式美学·第十八期·16:9 ·参数 --ar 16:9 --s 100 --c 0 --v 8.2）

> 中文：
16:9 横幅胸像，女子位于右侧三分区，面部略向右上方抬起，两只抬起的手伸入画面中央。开放的左半画面包含冷蓝建筑纵深、左下方粉蓝花朵散景，以及横向飞向面部的半透明珊瑚红玻璃蝴蝶；近侧耳边垂着一枚蓝色蝴蝶饰品，另一只发光蝴蝶悬停在远侧手边。她目光遥远向上，珊瑚色嘴唇轻合，松散黑色波浪发收进华丽高髻，并配蓝水晶和金色植物饰件。体积化数字肖像表现，面部渐变连续、反射关系真实、珠饰清晰并具镜头景深。珍珠象牙色与烟蓝真丝带香槟金浮纹，透明欧根纱袖口与主体布料清楚分离。右上方暖阳穿过叶隙和蝴蝶翅膀，在额头、眼睛与面颊投下有结构的影带；柔和冷补光保留面部塑形。横向前景从左侧冷蓝花朵过渡到中央暖色蝴蝶光，光晕只限于发光翅膀、珠宝和欧根纱边缘。

> 负面：避免蝴蝶数量失控、蝴蝶遮住双眼、双手被裁切、额外手指或手掌融合、发光扩散覆盖整张脸、蓝花与蝴蝶糊成光团、白棚光、布料漂白、哑光插画感、棕褐或橄榄色偏、界面文字与水印。

> 结构拆解：镜头=16:9横幅胸像｜场景=冷蓝建筑纵深+粉蓝花散景｜人物=华丽高髻+蓝水晶+金植物饰件｜动作=双手抬起入画，蝶横向飞向面部｜光色=右上暖阳影带+冷补光｜避错=蝶数量可控、手不裁、光晕不外溢

**GPT-Image 实测 CASE 08：云天裸肩回眸与巨幅衣袖**（中式美学·第十八期·16:9 ·参数 --ar 16:9 --s 100 --c 0 --v 8.2）

> 中文：
16:9 横幅后侧三分之二肖像，女子固定在右侧三分区，裸露的上背与近侧肩膀形成中央曲线，头部越肩转向观者。面部和完整银色发冠都收在右上方；一只巨大的珍珠质感衣袖从最左边缘高高升起，其余长袍像宽阔反光波浪一样横贯画面下部，形成由左向右的长水平动势。开放的左上区域呈现层层低饱和蓝天和立体奶油色积云。她以警觉的侧目看向镜头，珊瑚色嘴唇轻合；亮泽黑发盘成雕塑感高髻，配银色花卉金工、珍珠链和水晶流苏。体积化数字肖像表现，面部渐变连续、反射连贯并具镜头景深。大面积象牙色真丝承载流动的冷银、薄雾蓝、低饱和青绿与克制桃色织纹，而非平面图案。左上方清晰暖日光中等偏硬，并由冷色天空补光平衡背部、颈部和内侧衣褶。镜面光晕只限于银饰和抬起的丝绸脊线；皮肤保持桃色暖意，每条主要袖边都完整可见。

> 负面：避免裸背与肩部结构错误、越肩回望改成正面站姿、银色发冠被裁切、巨幅衣袖边缘缺失、衣褶融合成塑料波浪、天空过曝、白棚光、布料漂白、哑光插画感、棕褐或橄榄色偏、界面文字与水印。

> 结构拆解：镜头=16:9横幅后侧三分之二｜场景=低饱和蓝天+奶油积云｜人物=雕塑高髻+银饰+水晶流苏｜动作=越肩回眸，巨袖横贯成水平动势｜光色=左上暖日+冷天空补光｜避错=肩部结构对、袖边完整、天空不过曝

**GPT-Image 实测 CASE 09：珊瑚桃色侧身肖像**（中式美学·第十八期·16:9 ·参数 --ar 16:9 --s 100 --c 0 --v 8.2）

> 中文：
16:9 横幅腰部以上三分之二侧身肖像，女子位于左侧三分区，躯干朝向画面右侧，面部回转看向观者。巨大的花卉织锦肩部向画面中央推进，桃色欧根纱衣袖沿最左前景柔和升起，右侧三分区打开为暖色室内，珊瑚纹样帷幔和模糊花形逐渐化入散景。她平静直视并带极浅微笑，珊瑚色嘴唇轻合；亮泽黑发盘成圆润宫髻，以一朵超大淡桃牡丹、小珊瑚花、金丝、珍珠和水晶链构成发冠。体积化数字肖像表现，面部渐变连续、反射连贯、珠饰清晰并具镜头景深。灰蓝织锦上有超大象牙与珊瑚牡丹、低饱和青绿色叶片和凸起香槟金线，并与半透明桃色欧根纱清楚分层。右上方中等柔度暖光塑造面颊、鼻梁和肩部，深橄榄卡其背景保留丰富中间调。横向色彩路径从左侧发光桃色经过蓝珊瑚刺绣，最后落入右侧克制暖影；光晕只停留在线绣、珠饰和欧根纱边缘。

> 负面：避免人物位置移到右侧、侧身方向错误、超大桃色牡丹被裁切、肩部织锦糊成印花、背景整体发黄或发绿、皮肤磨成塑料、白棚光、布料漂白、哑光插画感、棕褐或橄榄色偏、界面文字与水印。

> 结构拆解：镜头=16:9横幅腰部以上三分之二侧身｜场景=暖色室内+珊瑚帷幔散景｜人物=圆润宫髻+超大淡桃牡丹发冠｜动作=侧身回转，织锦肩推进画面中央｜光色=右上暖光+横向色彩路径｜避错=人物位置在左、侧身方向对、牡丹不裁切

**Midjourney 实测 CASE 10：山涧嬉戏（田园风）**（MJ 英文+中文对照·--no 负面列表）

> 英文（直接喂 MJ）：
Cinematic pastoral photograph, three young adult East Asian women playing in a shallow mountain stream, delicate faces, slim figures, long black hair in loose braids, simple cream blue and sage cotton-linen dresses, natural laughter, splashing water, mossy rocks, forest sunlight, warm highlights, muted greens, soft haze, subtle 35mm film grain, realistic candid wide shot --no text watermark logo crowds dense buildings palace clusters small pavilions stacked roofs excessive carvings ornamental clutter busy background neon cyberpunk modern technology

> 中文翻译：一幅充满田园风情的电影感照片，三位年轻的东亚女性在浅浅的山涧溪流中嬉戏。她们面容清秀，身材纤细，长长的黑发随意地编成辫子，身着简洁的米蓝色和鼠尾草绿色棉麻连衣裙，脸上洋溢着自然的笑声，溪水飞溅，苔藓覆盖的岩石在阳光的照耀下闪闪发光。温暖的高光、柔和的绿色、朦胧的薄雾，以及35毫米胶片特有的细腻颗粒感，构成了一幅真实自然的广角画面。

> --no 负面列表：text, watermark, logo, crowds, dense buildings, palace clusters, small pavilions, stacked roofs, excessive carvings, ornamental clutter, busy background, neon, cyberpunk, modern technology

**Midjourney 实测 CASE 11：野花草地独影（田园风）**（MJ 英文+中文对照·--no 负面列表）

> 英文（直接喂 MJ）：
Cinematic pastoral photograph of one young adult East Asian woman alone in a wildflower meadow, delicate face, slim figure, long black hair, simple cream cotton-linen dress, holding wildflowers, tall grass moving in the wind, distant green hills, golden sunset backlight, muted green and cream film colors, soft haze, subtle 35mm grain, realistic natural skin --no text watermark logo crowds dense buildings palace clusters small pavilions stacked roofs excessive carvings ornamental clutter busy background neon cyberpunk modern technology

> 中文翻译：一张充满田园风情的电影感照片，一位年轻的东亚女性独自漫步在野花盛开的草地上。她面容清秀，身材纤细，一头乌黑的长发，身着简洁的米色棉麻连衣裙，手捧野花，高高的草丛随风摇曳，远处是翠绿的山峦，夕阳的金色光芒映照着她。照片色彩柔和，以绿色和米色为主，略带朦胧感，35毫米胶片的颗粒细腻，肌肤呈现自然逼真的效果。

> --no 负面列表：text, watermark, logo, crowds, dense buildings, palace clusters, small pavilions, stacked roofs, excessive carvings, ornamental clutter, busy background, neon, cyberpunk, modern technology

**武侠实测 CASE 12：雾中竹林骑马（次世代游戏CG·MJ系英文原版）**（@自热包·抖音实测·无负面词·正向锁定写法）
...
> 中文翻译：
宽幅全画幅电影镜头：一位身着中国古代武侠服饰、头戴宽檐斗笠的年轻女子，在雾气弥漫的竹林中骑马。马匹缓步前行或微微扬起前蹄，女子平静地将长剑持于身侧，长发与白衣自然飘动。人物在强烈逆光下呈现部分剪影，柔和的轮廓光勾勒衣袍、发丝与马匹的边缘。不要面部特写，人物不看镜头。米白与雾灰的分层衣袍、飘逸宽袖、垂坠感面料，简洁优雅的廓形，无繁复纹样，克制的仙侠美学。高耸茂密的竹林填满整个画面，浓雾弥漫，落叶盘旋。强烈的暖金绿色逆光、弥散朦胧的光晕，低饱和橄榄色调，宁静诗意的氛围，强纵深感，构图简洁不杂乱。次世代游戏 CG，超写实 PBR 材质，超清晰纹理，丰富的 SSS 次表面散射，逼真的布料飘动与马匹毛发，氛围雾气，精致的中式美学，高细节。

> 结构拆解（七段）：
镜头=宽幅全画幅电影镜头｜场景=雾中竹林（密竹填满全幅+浓雾+盘旋落叶）｜人物=武侠女子+宽檐斗笠+米白雾灰分层白衣+宽袖垂坠｜动作=骑马缓行/微扬蹄+长剑侧持+长发白衣飘动｜光影=强暖金绿逆光剪影+轮廓光勾边（衣袍/发丝/马缘）+弥散光晕｜约束=正向锁定"无面部特写、人物不看镜头"+克制仙侠美学（无繁复纹样）｜画质=次世代游戏CG+PBR材质+SSS次表面散射+超清晰纹理+高细节

> 方法亮点：
1. "不看镜头+无面部特写"直接写进正向词（库内负面词有"人物看镜头"，此作更进一步在正向锁定）
2. 逆光不追求亮部细节，用"部分剪影+轮廓光逐项勾边"（衣袍/发丝/马缘）控氛围
3. 低饱和橄榄色调+暖金绿逆光=色彩双锚点
4. 画质锚点用游戏CG系：PBR/SSS/超清晰纹理（与GPT-Image"体积化数字肖像"异曲同工）

**武侠实测 CASE 13：雾中夜行·提灯侠女**（V5.20 入库·暖黄灯光是唯一亮色）

> 中文：宽幅全画幅电影镜头：一位身着米白与雾灰分层衣袍的年轻武侠女子，头戴宽檐斗笠，在夜晚雾气弥漫的竹林中行走。她手提一盏暖黄纸灯，光只照亮身前一小片地面，其余身影隐入浓雾；长发与宽袖在夜风中自然飘动，人物在暗蓝夜色与灯光交界处呈现部分剪影，柔和轮廓光勾勒斗笠边缘、发丝与衣袍飘摆。无面部特写，人物不看镜头。低饱和青灰色调，暖黄灯光是画面唯一亮色，弥散光晕停留在灯纸与雾气交界。次世代游戏CG，超写实PBR材质，SSS次表面散射，真实布料飘动，氛围雾气，高细节，克制仙侠美学。

> 英文：Wide full frame cinematic shot, a young woman in ancient Chinese wuxia attire with a wide bamboo hat, walking through a misty bamboo forest at night. She carries a warm yellow paper lantern, the light pooling on the ground just ahead, the rest of her figure dissolving into dense fog. Long hair and wide sleeves flow naturally in the night breeze, figure partially silhouetted at the boundary between dark blue night and lantern light, soft rim light tracing the hat edge, hair and flowing robes. No facial close up, character does not look at camera. Low saturation blue-gray palette, warm lantern light as the only accent, diffused glow clinging to paper and fog. Next gen game CG, hyper realistic PBR materials, rich SSS, realistic flowing fabric, atmospheric haze, high detail, restrained immortal wuxia aesthetic.

> 要点：灯光破夜雾=唯一亮色锚点；剪影留在光雾交界；其余与 CASE 12 同配方（正向锁定不看镜头/次世代CG画质）。

**武侠实测 CASE 14：暗夜金纹·逆光回眸**（V5.20 入库·暗黑武侠基调 V7 示范）

> 中文：宽幅全画幅电影肖像：年轻女子身着带有精致金色暗纹的深色古风长袍，立于漆黑背景前，只由一道冷青逆光勾勒身形。发丝凌乱飘散，金色纹饰在逆光下微亮，她微微回眸，眼神沉静，目光越过观者；面部半入阴影，边缘被细窄轮廓光勾亮。大面积黑色负空间，低饱和青黑与哑金双色，弥散光晕只停留在金线与发丝边缘。次世代游戏CG，超写实PBR材质，金属与布料质感分明，SSS次表面散射保留皮肤通透，高细节，克制冷峻美学。

> 英文：Wide full frame cinematic portrait, a young woman in a dark ancient Chinese robe with subtle gold patterns, standing against a pure black background, outlined by a single cool cyan backlight. Hair drifting loose and wild, gold patterns faintly catching the light, she turns her head slightly, calm gaze toward the camera but looking past the viewer. Face half in shadow, thin rim light tracing the edges. Large black negative space, low saturation blue-black and muted gold palette, diffused bloom only on gold threads and hair strands. Next gen game CG, hyper realistic PBR materials, distinct metal and fabric textures, rich SSS for translucent skin, high detail, restrained cold aesthetic.

> 要点：冷青逆光单光源+哑金暗纹；大面积黑负空间；"目光越过观者"=有眼神但不直视，替代"不看镜头"的另一种氛围锁法。

**武侠实测 CASE 15：竹林深径·伸手挽留**（V5.20 入库·双人张力叙事位示范）

> 中文：宽幅全画幅电影镜头：清晨雾气笼罩的竹林深径，一位身着浅金色华服的女子快步向前，伸手拉住前方背身而立的深衣男子衣袖，衣袂被风带起；两人均为背影或侧身，在逆光下呈剪影，柔和轮廓光勾勒衣袍边缘与飘动的发带。竹子高耸填满画面，浓雾在两人之间流动，落叶盘旋。低饱和橄榄与暖金绿色调，弥散光晕，宁静诗意的氛围中带一丝张力。无面部特写，人物不看镜头。次世代游戏CG，超写实PBR材质，真实布料飘动，氛围雾气，高细节。

> 英文：Wide full frame cinematic shot, a misty bamboo path at dawn, a young woman in a light gold ancient robe steps forward and reaches out to hold the sleeve of a man in dark robes standing with his back turned, robes caught in the wind. Both seen from behind or three-quarter, figures silhouetted against strong backlight, soft rim light tracing robe edges and drifting hair ribbons. Dense towering bamboo filling the frame, fog flowing between the two figures, swirling leaves. Low saturation olive and warm golden-green palette, diffused glow, quiet poetic atmosphere with a hint of tension. No facial close up, characters do not look at camera. Next gen game CG, hyper realistic PBR materials, realistic flowing fabric, atmospheric haze, high detail.

> 要点：挽留张力位（一前一后/背身+伸手）；两人全剪影，张力靠衣袂动态+距离感，雾作情绪隔层。

**武侠实测 CASE 16：竹林策马·暖金逆光**（V5.20 入库·CASE 12 暖金变体）

> 中文：宽幅全画幅电影镜头：武侠女子头戴宽檐斗笠，策马于黄昏暖雾的竹林中，马匹四蹄微扬，衣袍与马尾在疾风中拉成斜线。人物与马在暖金色强逆光下呈深色剪影，柔和轮廓光勾勒斗笠、衣袍与马匹肌肉边缘；雾被阳光染成金绿色，落叶如雨。低饱和金绿橄榄色调，弥散光晕，宁静而有力的氛围。无面部特写，人物不看镜头，画面简洁，强纵深。次世代游戏CG，超写实PBR材质，SSS次表面散射，真实毛发与布料，高细节，精致中式美学。

> 英文：Wide full frame cinematic shot, a young woman in wuxia attire with a wide bamboo hat galloping through a misty bamboo forest in warm dusk light, the horse rearing slightly, robe and tail streaming into diagonal lines in the wind. Figure and horse as deep silhouettes against strong warm golden backlight, soft rim light tracing the hat, robe and horse muscle edges. Fog tinted golden-green by sunlight, leaves falling like rain. Low saturation golden-green olive palette, diffused glow, quiet yet powerful atmosphere. No facial close up, character does not look at camera, uncluttered composition, strong depth. Next gen game CG, hyper realistic PBR materials, rich SSS, realistic fur and fabric, high detail, refined ancient Chinese aesthetic.

> 要点：暖金逆光+雾染金绿；马扬蹄斜线动势；黄昏版 CASE 12。
...
### V7 暗黑武侠·黑金冷峻（V5.20 入库·暗调武侠/权谋/复仇/对峙）

> 基调：大面积暗部负空间 + 冷青逆光 + 哑金暗纹，克制冷峻，不追求高光堆砌。

基调句（中文）：暗黑武侠基调，大面积黑色负空间，冷青逆光勾勒身形，金色暗纹在光下微亮，低饱和青黑与哑金双色，克制冷峻氛围。

基调句（英文）：Dark wuxia tone, large black negative space, cool cyan backlight outlining the figure, subtle gold patterns catching the light, low saturation blue-black and muted gold palette, restrained cold atmosphere.

适配：武侠权谋/复仇/对峙/夜行/暗杀题材；人物神色沉静、目光越过观者；光晕只停留在金线与发丝边缘。
...
### 光影五步写作法（V5.17 入库·策略层·先定策略再取词）

> 上公式管"光长什么样"（词库），本公式管"光怎么写进 prompt"（顺序策略）。来自 AI 电影感教程核心方法论：电影感不靠堆"电影感/大片感/8K"关键词，靠把光影写明白——光影材质决定画面"有多真"，光影才决定画面"是不是活的"。

**五步顺序**：

1. **先拒绝**：先否定平庸光，模型才知道边界——"拒绝如此平均的棚拍光，拒绝四面均匀照明"
2. **总体要求**：给光定调——"有重点、有方向、有氛围，能制造观众情绪的光"
3. **打光方法**：对谁打 + 光从哪来——"增强对人物主体的单光源打光，光照主要来自窗外单一正向光"
4. **空气介质**：让光"被看见"——"空气中增加粒子和灰尘，让光能被看见"（可替换词见下：尘埃微粒漂浮/丁达尔光束/耶稣光/雾气氤氲/蒸汽升腾）
5. **约束收尾**：AI 不能干什么——"禁止过曝与死黑，保持暗部细节，画面干净"

**进阶：信息图先行**——加光影前先把构图、信息、细节完整的画面描述写好（称为"信息图"），再叠光影段；这样画面里再小的细节加光后都不会丢。写光影时可测多种光再定稿，详写：光的角度和颜色、对画面材质的改变、曝光降低档位、明暗分布比例——这是最难写、AI 帮助最大的一段。

**完整示例（可直接抄）**：
拒绝如此平均的棚拍光。有重点、有方向、有氛围，能制造观众情绪的光。增强对人物主体的单光源打光，光照主要来自窗外单一正向光。空气中增加粒子和灰尘，让光能被看见。禁止过曝与死黑，保持暗部细节。
...
| 光质 | 柔光 / 漫射光 / 低反差 / 硬光 / 散射 |
| 光色 | 暖橙 / 琥珀色 / 金黄 / 蜜色 / 白中透暖 / 蓝紫混合色调 / 病态绿顶光 / 青绿冷光 / 翡翠冷光 / 荧光绿 |
| 明暗 | 高光不溢出 / 暗部保留细节 / 光比柔和 / 面部受光均匀 / 强烈明暗对照（chiaroscuro）/ 极端明暗对比 |
| 光效 | 光晕 / 丁达尔光束 / 耶稣光（光柱倾泻在人物脸上与衣摆）/ 光斑虚化 / 尘埃微粒漂浮 / 烟尘弥漫 / 尘烟浮动 / 烛火摇曳 / 垂直天光（一道光柱自天际垂落）/ 云隙光束斜洒 / 动态残影（faint motion ghosting）/ 半透明层次（translucent layers）/ 玻璃质感高光 |
...
**张力叙事位（挽留/错过/告别·武侠/剧情向·V5.20 入库）**

| 关系位 | 写法 |
|---|---|
| 背身挽留 | 女子快步向前伸手拉住背身男子的衣袖，衣袂被风带起（she steps forward and catches the sleeve of the man turning away, robes caught in the wind）|
| 伸手未及 | 一人伸手向前，指尖与另一人衣角差一寸，雾隔在两人之间（fingertips a breath away from the robe, mist drifting between them）|
| 背道相错 | 两人各向画面两端走去，衣袍与发带向相反方向飘动（walking away from each other, robes and ribbons streaming in opposite directions）|

> 用法：张力位+雾/光做情绪隔层；人物可全剪影，张力靠"衣袂动态+距离感"，不一定给脸。
...
| 略带悲伤 | 黄昏暖色空间逆光 | 柔光 | 暖橙/琥珀 | "暮色从身后漫入，人影沉在暖光与阴影之间" |
| 危险逼近 | 冷月高位侧逆光 | 硬光 | 冷白/青蓝 | "冷月从斜上方打下，轮廓如刀，阴影拉长" |
| 诡异童话/舞台剧 | 病态绿顶光 | 硬光 | 病绿/青绿 | "绿光自头顶直落，表情在光影里失真" |

> ⚠️ 反面教材（V5.17 补充）：均匀棚拍光 = 画面平、无层次，像航拍冷静客观。电影感的第一步往往就是"拒绝平均光"——先让光有方向、有明暗差。
...
| 夜雾 | 夜晚浓雾漫过林间水面，仅灯光或月色破雾，人物半隐 | 神秘、孤寂 | 夜行 / 提灯 / 竹林夜雾 / 暗杀 |
...
### 建筑风格库专章（V5.23 入库·建筑类型/屋顶形制/结构构件/材料质感/光效句）

> 用法：写建筑先定类型（宫殿/园林/寺庙…），再给屋顶形制（庑殿/歇山/攒尖…），补结构构件与材料质感，最后收一条建筑光效句。

**① 建筑类型（先定是什么建筑）**

| 类型 | 制式要点 | 点题词 |
|---|---|---|
| 宫殿 | 大殿/宫阙/丹墀/廊庑，重檐庑殿顶，琉璃瓦，红墙汉白玉台基 | 金銮殿 / 龙陛 / 椒房 / 承露台 |
| 园林 | 亭台楼阁榭舫，曲廊，假山池沼，借景框景 | 水榭 / 画舫 / 回廊 / 月洞门 |
| 宅院 | 四合院/合院/府邸，门楼影壁，正房厢房倒座 | 朱漆大门 / 影壁 / 垂花门 / 抄手游廊 |
| 寺庙道观 | 山门/大殿/钟鼓楼/塔/经幢，攒尖或歇山顶 | 大雄宝殿 / 钟楼 / 经幢 / 香炉 |
| 城防 | 城墙/城门/角楼/瓮城/箭楼 | 城门洞 / 敌楼 / 雉堞 |
| 市井 | 牌楼/商铺/茶楼/酒肆/戏台 | 牌坊 / 招幌 / 戏楼 / 花灯棚 |
| 高塔 | 木塔/砖塔/密檐塔/楼阁式塔 | 浮屠 / 宝塔 / 塔铃 |
| 桥梁 | 拱桥/廊桥/石桥/九曲桥 | 石拱 / 廊桥 / 阑干 |

**② 屋顶形制（制式核心·等级感）**

| 形制 | 特征 | 适用 |
|---|---|---|
| 庑殿顶 | 五脊四坡，最高等级，正脊吻兽 | 宫殿主殿 |
| 歇山顶 | 九脊，四坡加山花，次于庑殿 | 大殿/楼阁 |
| 悬山顶 | 双坡出檐悬挑 | 民居/祠 |
| 硬山顶 | 双坡山墙封顶 | 普通民居 |
| 攒尖顶 | 锥形聚顶，宝顶收束 | 亭/塔 |
| 卷棚顶 | 弧形脊，柔和不设正脊 | 园林游廊 |
...
> 屋脊装饰：正脊鸱吻 / 垂脊蹲兽 / 脊刹 / 宝顶

**③ 结构构件（细节层）**

斗拱 / 梁枋 / 檐柱 / 雀替 / 藻井 / 天花 / 台基（须弥座）/ 栏杆望柱 / 挂落 / 门簪 / 铺首 / 门钉 / 窗棂（格扇/直棂/回纹/冰裂纹）/ 照壁影壁 / 抱鼓石 / 柱础

**④ 材料质感（材质层）**

青砖黛瓦 / 琉璃瓦（黄/绿/蓝）/ 红墙 / 白墙（徽派马头墙）/ 木构（楠木/樟木/杉木）/ 汉白玉台基 / 青石地面 / 夯土墙 / 金砖墁地 / 包浆木门
...
| 题材 | 建筑选型 |
|---|---|
| 大唐盛世/朝堂权谋 | 宫殿·重檐庑殿·琉璃瓦·汉白玉台基 |
| 世家宅斗/闺阁 | 宅院·垂花门·抄手游廊·影壁 |
| 文人雅集/园林 | 园林·水榭·曲廊·月洞门·卷棚顶 |
| 边塞战争 | 城防·城墙·角楼·雉堞 |
| 禅意江湖 | 寺庙·山门·大殿·经幢·塔铃 |
| 市井烟火 | 市井·牌楼·茶楼·戏台·招幌 |
...
### GPT-Image 数字人像负面范式（V5.18 入库·画面反推式·非原始生成参数）

> 作者做法：不用通用负面，而是"画面反推"——对着生成图找最容易翻车的点，逐条封死。通用段 + 专属构图负面两段式。

**通用段（数字人像/织锦/欧根纱类必加）**：
白棚光, 布料漂白, 哑光插画感, 棕褐或橄榄色偏, 界面文字与水印, 塑料肤质, 过度磨皮

**专属构图负面句法（按图反推，逐条封死）**：
避免{主视觉道具}被裁切、{遮挡关系}、{结构错误}、{颜色/材质翻车}——例："避免巨型牡丹袖被裁切、花团遮住双眼、额饰缺失、手臂与袖子粘连、刺绣变成平面印花"

**用法**：通用段直接拼；专属段按当前画面"最贵的三样东西"反推（主道具/视线/留白），写清"不要发生什么"。
...
### 兵器库专章（V5.21 入库·武器/暗器/纹饰纹理/制式长短·词库+写法）

> 用法：每个镜头 1 件兵器点题；写兵器三步——先定制式长短，再给材质纹理纹饰，最后收一条光效互动句（拼进手中道具公式）。

**① 制式样式与长短（先定形制再描写）**

| 大类 | 制式·长短 | 点题雅词 |
|---|---|---|
| 剑 | 长剑（三尺青锋）/ 短剑（匕首）/ 重剑（双手巨剑）/ 软剑（腰剑）/ 佩剑 / 君子剑 / 断水剑 | 三尺青锋 / 松纹古剑 / 龙泉 / 承影 / 含光 |
| 刀 | 唐刀 / 横刀 / 环首刀 / 苗刀 / 朴刀 / 柳叶刀 / 雁翎刀 / 弯刀 / 大环刀 / 短刀 | 百炼钢刀 / 陌刀 / 霜刃 |
| 枪矛 | 红缨枪 / 丈八蛇矛 / 方天画戟 / 戈 | 枪出如龙 / 一点寒芒先到 |
| 长兵重兵 | 齐眉棍 / 狼牙棒 / 月牙铲 / 禅杖 / 铁鞭 / 双锏 / 长柄锤 / 流星锤 | 降魔杵 / 开山之势 |
| 钩叉拐 | 护手钩 / 双钩 / 三股叉 / 铁叉 / 拐 / 鸳鸯钺 | 勾魂夺魄 |
| 弓弩 | 长弓 / 反曲弓 / 角弓 / 铁胎弓 / 连弩 / 袖弩 / 诸葛连弩 | 箭如飞蝗 / 弓开满月 |
| 奇门兵器 | 判官笔 / 峨眉刺 / 铁扇 / 铁笛 / 铁拂尘 / 双节棍 / 三节棍 / 飞爪 | 判官笔点穴 / 出其不意 |

**② 暗器（小型投射·贴身暗藏）**

| 类型 | 点题词 | 材质/光效句 |
|---|---|---|
| 飞针类 | 银针 / 金针 / 淬毒针 / 暴雨梨花针 | 细如牛毛，寒光一闪即逝 |
| 飞刀镖类 | 柳叶飞刀 / 梅花镖 / 十字镖 / 金钱镖 / 铁莲子 / 飞蝗石 | 脱手即隐入夜色 |
| 袖箭机簧 | 袖箭 / 袖弩 / 吹箭 / 铁蒺藜 | 机簧一扣，无声出膛 |
| 绳爪链类 | 飞爪 / 绳镖 / 流星镖 | 铁链缠绕腕间，爪尖一点冷光 |

**③ 纹饰纹理质感（装饰工艺层·词库）**

| 层级 | 词库 |
|---|---|
| 剑身部件 | 剑脊 / 剑格护手 / 剑穗 / 血槽 / 剑首 / 剑柄缠丝 / 包浆 |
| 刀身部件 | 刀背 / 反刃 / 吞口兽首 / 刀柄缠绳 / 铜护挡 |
| 锻造纹理 | 百炼钢纹 / 折叠锻打纹 / 水波纹 / 云纹 / 松纹 / 鱼鳞纹 / 龟背纹 |
| 装饰纹样 | 云雷纹 / 饕餮纹 / 螭龙纹 / 夔龙纹 / 回纹 / 忍冬纹 / 卷草纹 / 缠枝纹 / 宝相花 / 唐草纹 / 兽面纹 / 弦纹 / 菱格纹 / 万字纹 |
| 工艺质感 | 错金 / 错银 / 鎏金 / 嵌宝石 / 描金 / 黑漆古 / 精钢哑光 / 包浆温润 / 锈蚀古剑 / 刃口寒光 |
...
**⑤ 兵器×人物服饰造型妆造配套（V5.22 入库·兵器决定人物造型）**

> 用法：人物带兵器时，造型从本表取一行配套，避免"拿着剑却穿书生袍"式的错配。

| 兵器系 | 服装造型 | 发型妆造 | 配饰 | 气质基调 |
|---|---|---|---|---|
| 剑客（长剑/软剑/佩剑） | 月白/青灰劲装，外罩轻薄外披，宽袖内扎窄袖口 | 高束马尾/半束发，发带飘垂，眉目清冷或坚毅 | 腰间长剑，剑穗，束发冠或额间花钿 | 清逸孤傲，衣袂带风 |
| 重剑/巨剑 | 玄黑/深褐短打，皮质护腕护臂，宽肩束腰 | 短发束起或散发微乱，面容粗犷坚毅 | 背后或拄地巨剑，金属护肩 | 沉重力沉，不动如山 |
| 刀客（唐刀/苗刀/雁翎刀） | 深色窄袖短打，束腕，腰系刀带 | 利落单马尾/高髻，眼神锐利 | 腰侧横刀，皮革护手 | 干脆利落，杀气内敛 |
| 枪将（红缨枪/方天画戟） | 玄甲战袍/束身战衣，护心镜 | 束发冠/凤翅盔，剑眉英气 | 红缨呼应战袍，肩甲 | 英武挺拔，战意凛然 |
| 弓手（长弓/角弓/连弩） | 青绿/褐灰短打，窄袖，下摆收束 | 束发+抹额或头巾，神色警觉 | 背箭袋，扳指，护臂 | 山林猎手，静若处子 |
| 暗器刺客（飞针/袖箭/飞爪） | 夜行衣/黑青紧身，低调无纹饰 | 全束发，蒙面或兜帽，眼神锐利 | 腰带暗藏暗器，腕部袖箭 | 来去无声，杀气藏于暗处 |
| 奇门（判官笔/铁扇/峨眉刺） | 文士袍/道袍，或劲装外披大氅 | 高髻纶巾/半束发，神色温雅 | 铁扇别腰，笔挂腰侧，玉佩 | 温雅藏锋，谈笑定生死 |
| 行者棍僧（齐眉棍/禅杖/月牙铲） | 灰色/褐色僧袍，宽袖扎起 | 光头或束发，眉目沉静 | 念珠，禅杖锡环 | 沉稳内敛，禅意与威压并存 |

**⑥ 武器光效大全（按兵器系·V5.22 入库）**

| 兵器系 | 光效互动句 |
|---|---|
| 剑 | 出鞘一线寒光，刃口反光只停在锋线 / 月光下剑身如水，烛火映过剑脊 / 软剑抖开，剑身泛起银鳞波光 |
| 刀 | 刀身如水，吞口兽首微亮 / 弯刀出鞘，弧线反光如一弯月牙 / 劈砍时刀光成弧，残影留一线 |
| 枪矛戟 | 枪尖一点银光，红缨风中拂动 / 方天画戟月牙刃反光，寒芒在戟尖流转 |
| 重兵 | 铁鞭沾水，水珠沿棱线滑落 / 狼牙棒钉刺反光如星点 / 巨剑拄地，剑身映出烛火一线 |
| 弓弩 | 弓开满月，弦线反光一线 / 箭尖一点寒芒，箭羽没入光晕 / 连弩机簧寒光，箭雨如蝗 |
| 暗器 | 暗器脱手瞬间，寒星划过 / 银针细如牛毛，寒光一闪即逝 / 飞刀旋转反光成环，绳镖链光游走 |
| 奇门 | 铁扇合拢扇骨齐鸣，冷钢扇边一线白光 / 判官笔笔尖一点寒星 / 峨眉刺双刺交叉，寒光两道 |
| 通用氛围 | 包浆温润仅刃脊一线反光 / 血槽暗影吞光 / 锈蚀古剑哑光无锋，历史感 |

**⑦ 剑术招式与持剑法（V5.24 入库·传统剑术规范·写持剑人物用）**

> 用法：写持剑动作三步——先定持剑法（握法），再给步型+剑法（攻防动作），最后左手剑指辅势+目视方向收尾。搜「剑招」「剑术」「持剑」调出。

**持剑四法（握剑手法·防"握剑握成刀"式错配）**

| 持剑法 | 手法 | 适用 |
|---|---|---|
| 满把 | 全掌握柄，五指环握剑柄，虎口贴靠剑格 | 劈/砍/架等发力动作 |
| 钳把 | 食指拇指与虎口钳住剑柄，其余三指自然附柄 | 刺/点等精准动作 |
| 刁把 | 虎口刁牢剑柄，拇指食指中指自然伸直虚附，其余两指基本离开 | 撩/挑等腕部动作 |
| 压把 | 无名指与小指放开，以指节压于剑柄后端 | 剑尖下垂/虚招诱敌 |

**剑指规范（左手辅势·传统剑术标配）**：食指与中指伸直并拢，无名指与小指屈向手心，拇指压在无名指指甲上，手腕反屈，手背朝上或朝前，指尖指向目标方向（护胸/指敌/上架三用）。

**剑法十六字**：刺、劈、点、撩、挑、崩、截、斩、抹、削、云、挂、架、压、绞、扫。

**常用剑招（可直接写进 prompt 的动作段）**

| 剑招 | 动作描述 | 力达部位 | 剑指辅势 |
|---|---|---|---|
| 起势·含光抱月 | 并步或弓步站立，剑藏于袍或持于身侧，剑尖斜指地面，神光内敛，气沉丹田，蓄势待发 | — | 剑指贴左臂或护胸 |
| 弓步直刺 | 上步成弓步，右手持剑向前直刺，剑与臂成一直线，力透剑尖 | 剑尖 | 剑指护于胸前或甩至身后 |
| 弓步撩剑 | 剑由后向前撩出，剑尖朝上，身体随转体带动 | 剑刃前段 | 剑指上架于额前 |
| 并步点剑 | 并步站稳，剑尖前点，腕垂臂稳，腕部急抖如蜻蜓点水 | 剑尖 | 剑指附右腕 |
| 提膝劈剑 | 一腿上提，剑向下劈，势沉力猛 | 剑刃中段 | 剑指护胸 |
| 旋腕推剑 | 右腕外旋，剑身横推于身前，虎口对剑脊，剑尖指向敌人 | 剑尖 | 剑指护胸指敌 |

**剑招写作公式**：步型（弓步/虚步/丁步/提膝）+ 持剑法（满把/钳把/刁把）+ 剑法动词（刺/撩/点/劈）+ 力达部位（剑尖/剑刃）+ 左手剑指辅势（护胸/指敌/上架）+ 目视方向（视剑尖/视敌人/视剑指）。

> 示例：弓步满把直刺，剑与臂成一线力透剑尖，左手剑指护于胸前指敌，目视剑尖方向。

**⑧ 武术基础·步型拳掌腿法·招式库（V5.25 入库·写打斗/练武/对峙人物用）**

> 用法：打斗动作三步——先定步型（站桩根基），再给拳掌腿手法（攻防动作），最后收身法+目视方向；招式名自带画面感，可直接点题。搜「武术」「步型」「拳法」「招式」调出。

**步型五式（武术根基·写站姿/对峙先定步型）**

| 步型 | 动作规范 | 画面感 |
|---|---|---|
| 弓步 | 前腿屈膝半蹲，大腿接近水平，脚尖微内扣、膝不过脚尖；后腿挺膝蹬直，脚尖外展约45°，全脚掌着地；挺胸立腰，前弓后绷 | 前冲蓄力，如箭在弦 |
| 马步 | 两脚平行开立约脚长三倍，屈膝半蹲，膝不过脚尖，挺胸立腰 | 稳如磐石，重心下沉 |
| 虚步 | 后腿屈膝半蹲（大腿近水平），脚尖外展45°；前腿微屈，脚面绷直、脚尖虚点地 | 含虚待发，进退皆可 |
| 仆步 | 一腿全蹲，大小腿贴紧，全脚掌着地，膝与脚尖外展约45°；另一腿伸直仆地，脚尖内扣 | 下盘横扫，身低势沉 |
| 歇步 | 两腿交叉，屈膝全蹲，前脚全掌着地，后脚脚跟提起 | 拧身蓄劲，暗藏转折 |

**拳掌腿法（攻防动作·写实招式）**

| 类别 | 招式 | 力点/要领 |
|---|---|---|
| 拳法 | 冲拳 | 拳面为力点，直线向前冲击，臂与拳成一线 |
| 拳法 | 崩拳 | 前臂由屈到伸，拳背或拳面短促发力，寸劲 |
| 拳法 | 贯拳 | 拳眼朝上，由侧向弧线贯击，力达拳面 |
| 拳法 | 斩拳 | 拳轮为力点，前臂由上向下斜面斩击 |
| 拳法 | 挑拳 | 拳面为力点，由下向上屈肘上挑，高不过头 |
| 掌法 | 推掌 | 掌根为力点，向前直线推出，力达掌心 |
| 掌法 | 劈掌 | 掌外缘为力点，由上向下劈落 |
| 掌法 | 穿掌 | 指尖为力点，沿对方手臂上方前穿 |
| 掌法 | 托掌 | 掌心向上托起，力达掌心，护头/托肘 |
| 掌法 | 按掌 | 掌心向下按压，压制对方攻臂 |
| 掌法 | 截掌 | 中途截击对方攻击路线，破坏攻势 |
| 指法 | 标指 | 手指如标枪突刺，专攻穴位要害 |
| 指法 | 戳指 | 指尖用力戳击，攻眼、喉等脆弱部位 |
| 指法 | 点穴 | 指力精准点击穴位，使对手麻木失力 |
| 腿法 | 弹腿 | 屈伸性腿法，脚尖绷直，快速弹出 |
| 腿法 | 鞭腿 | 腿如鞭甩，以脚背横扫 |
| 腿法 | 扫堂腿 | 下盘横扫，腿贴地扫向对方脚踝 |
| 腿法 | 窝心脚 | 正蹬，力达脚跟/脚掌，攻心口 |
| 腿法 | 鸳鸯脚 | 左右连环踢，虚实相间 |
| 腿法 | 连环腿 | 连续踢击，一脚接一脚不留间隙 |
| 身法 | 翻腕 | 手腕翻转，改变攻击方向 |
| 身法 | 沉肘顶肘 | 肘部下沉护肋/向前顶击 |
| 身法 | 靠肩 | 肩部贴身靠打，近身发力 |
| 身法 | 擒拿锁腕 | 扣腕反关节，分筋错骨锁脉 |

**太极名式（动作自带气韵·古风/仙侠通用）**：起势·两臂前举屈膝按掌 / 左右野马分鬃·抱球上步弓步分掌 / 白鹤亮翅·跟步抱球后坐转体虚步分手（右掌上提左掌下按，两臂成弧形）/ 搂膝拗步 / 云手·转体划弧撑掌收步 / 单鞭·出步勾手弓步推举 / 高探马 / 如封似闭 / 十字手。

**武侠招式名库（点题用·自带画面感）**：仙人指路 / 三环套月 / 大魁星式 / 燕子抄水 / 蜻蜓点水 / 金蛇盘柳 / 大鹏展翅 / 白鹤亮翅 / 拨草寻蛇 / 怀中抱月 / 宿鸟投林 / 乌龙摆尾 / 风卷莲花 / 狮子摇头 / 虎抱头 / 转身勒马 / 顺水推舟 / 流星赶月 / 推山掌 / 缠丝掌 / 分影掌。

**武打动词速查（写行为不写感受）**：撞、探、压、崩、锁、扣、引、擒、卸、震、吐、回锋、吞、截、撩、挂、扫、绞。

**打斗写作公式**：步型（弓步/虚步/马步）+ 招式（冲拳/推掌/弹腿）+ 力点（拳面/掌根/剑尖）+ 攻防目标（攻面门/截腕/扫下盘）+ 身法收尾（翻腕/沉肘/侧身）+ 目视方向（视对方眼神/视拳势）。

> 示例：左弓步冲拳直取面门，拳与臂成一线力透拳面，右掌护胸，目光锁定对方眼神不闪。

**打斗实战拆招库（V5.25 入库·写交手过招用·一段一回合）**

> 用法：打斗写「回合」不写"打得激烈"——每回合 = 对手出招（一个可观察的征兆）+ 己方拆解（闪避/格挡/反击）+ 结果落点（借力/移位/兵器脱手）。先写征兆（刀未起右脚先碾半圈），再写反应（不硬接，矮身滑步钻到身后），判断先于速度。搜「打斗」「拆招」「交手」调出。

**拆招句式库（整句可抄·替换兵器/部位即可）**：
- 侧身堪堪避开劈来的刀锋，手肘顺势撞向对方肋下，扣住他握刀的手腕反向一拧，刀刃脱手砸落。
- 对方飞踹直扑面门，他不硬接，矮身滑步钻到对方身后，手掌按其后颈往下一压，膝盖同时顶向膝弯。
- 剑势未尽，他左爪已递到面前；不退反进，左肩一沉一耸，右腕疾转，剑挽寒光自奇诡角度反刺腰胁。
- 拔身一耸掠起数尺，空中拔剑，剑光一闪迎面刺出，双剑相交，"叮当"龙吟之声不绝。
- 掌力相接一触即分，借力飘开后翻，足尖点地即变虚步，掌势再起。

**身法步法（写移动/轻功）**：孤鸿掠水 / 落叶随风 / 鬼魅残影步 / 八步赶蝉 / 飘忽辗转 / 腾挪跳跃 / 贴地滑步 / 借力后翻。

**打斗写作公式**：征兆（对手出招前的微动）+ 应对（硬接/闪避/借力）+ 反击（招式+力点+部位）+ 结果（移位/脱手/失衡）+ 声效（金铁交鸣/破空/闷响）。

**兵器对打句式库（V5.27 入库·刀剑枪等兵器双人交手用·一回合一句）**

> 用法：兵器对打写「兵刃相交点+攻防方向+声效+破绽」，每回合一句，节奏即打斗（短句连击=激烈，长句=试探）。搜「兵器」「对打」「刀剑」「枪」调出。

**剑对剑**：
- 双剑相交，一声龙吟清越，剑身一触即分，两人各退半步，目光锁定对方剑尖。
- 他剑走轻灵，连刺三剑如银蛇吐信；对方剑势一沉，横剑格开，剑刃交击溅出几点火星。
- 一剑递到咽喉半寸处，对方侧身让过，剑锋贴着耳际滑开，削断一缕发丝。

**刀对剑**：
- 刀光如匹练劈下，他举剑格挡，刀剑相撞，震得虎口发麻；顺势卸力，剑尖自刀身滑过，直取对方手腕。
- 刀走刚猛，大开大合；剑走轻灵，不接其锋，游走间寻隙而进，剑尖点向刀背。

**枪对剑**：
- 长枪抖出一朵枪花直取面门，他侧身避过，枪尖擦面而过；进步欺身，剑贴枪杆削向握枪之手。
- 枪出如龙，连扎三枪逼得对方步步后退；剑客矮身一滚，自枪下钻入，剑光自下而上撩起。

**奇门对兵**：
- 铁扇合拢，扇骨点向对方腕脉；对方翻腕变招，剑尖反挑，两人兵刃一触，错身而过，交换了位置。
- 判官笔如蛇出洞，笔尖连点三处大穴；对方剑势展开成网，笔尖每次都被剑锋格开，金铁之声密如急雨。

> **兵器对打节奏公式**：试探（一触即分/虚晃）+ 强攻（连招+声效）+ 破绽（力尽/失位/脱手）+ 逆转（借力/近身/换位）。写激烈打斗用短句连击，写高手过招用长句缓推。
...
| 仙侠法器·发光系 | 灵剑 / 法剑 / 玉尺 / 宝镜 / 法器珠 / 荧光符箓 | 冷钢淬光 / 白玉含光 / 琉璃透光 | 剑身荧光微亮，辉光只停留在剑脊与雾的交界 / 法器悬空泛微光，光晕不溢满全镜 |
...
- 镜头：85mm + ARRI Alexa 35（相机参数），Kodak 胶片色彩 + 金色时刻光晕（风格锚点 ≤2）【净化状态 PASS】
...
14 相机参数+质感+风格：85mm f/1.4，ISO 400，低反差柔光 + 电影化（风格锚点 ≤2）；浅景深奶油散景（技术字段）；"柔和唯美细腻"四感基调不计配额；细微胶片颗粒并入低反差柔光质感；负面词：手部畸形、六指、五官错位、塑料质感、过度磨皮、强饱和、现代物件、文字水印
```
...
**景深三档速查（V5.17 补全·与「镜头光学质感速查表」联动）**：

| 档位 | 写法 | 效果 |
|---|---|---|
| 深景深 | 深景深，前后景全清晰，完整展现场景空间 | 史诗、全景交代、纪实 |
| 中等景深 | 中等景深，主体从环境分离，电影层次 | 叙事主力、平衡 |
| 浅景深 | 浅景深，背景柔和虚化，视觉集中在人物情绪 | 情感、特写、梦幻 |
...
### 道具路径构图法（V5.18 入库·巨袖/长道具/花枝·GPT-Image 九图实测）

> 原理：画面里"巨袖/横笛/花枝"等长形道具不只是装饰，而是引导视线的路径——先定人物落点，再让道具带着视线走。

**核心口诀**：

1. **先定人物落点，再让大袖形成路径**：人物固定左/右侧三分区，巨袖/衣袍从另一边入画、横穿画面下部，形成由一侧到另一侧的长水平动势
2. **长道具先定线**：写清起点和终点，再安排手——"横笛从左侧四分之一处起始，沿开放中心斜向延伸至唇边；一只手在中左部托住笛，另一只手在笛下舒展"
3. **花枝作自然桥梁**：枝条从前景开始，在人物上方/后方横向通过，贯穿蓝灰负空间（"一枝缀满白花的枝条从最左前景开始，在发冠上方横向通过，延伸到画面右侧"）
4. **负空间分层**：开放的左/右区域给层次（天空云层/建筑纵深/散景花朵），人物所在侧保持实焦

**落地模板**：16:9 横幅{景别}肖像，人物固定在{左/右}侧三分区，{道具路径}，开放的{负空间}区域{层次描述}。体积化数字肖像表现，面部渐变连续、反射连贯、织物细节清晰并具镜头景深。{定向暖光}塑造{面部/肩部}，{冷补光}分离主体，光晕只停留在{金属线/丝绸脊线/珠宝}上。
...
### 镜头光学质感速查表（V5.17 入库·色差/运动模糊/畸变/景深/散景·三档参数化）

> 五章管"镜头怎么动"，本表管"镜头本身什么光学质感"——同一运镜，换光学质感三档，电影感立即不同。按"无→轻→强"三档取词，或按情绪直接选档。

| 维度 | 无/默认（真实干净） | 轻微（光学感） | 强烈（风格化/冲击） |
|---|---|---|---|
| 色差 | 无色差，画面干净，高光边缘无色彩分离，真实镜头质感 | 轻微色差，画面边缘细微红蓝偏移，增加镜头光学感 | 强烈色差，明显色彩分离，复古胶片/实验电影效果 |
| 运动模糊 | 静态清晰，高速快门冻结运动瞬间，保留全部细节 | 轻微运动模糊，运动方向自然拖影，速度感 | 强烈运动模糊，大范围动态拖影，高速动作电影冲击 |
| 镜头畸变 | 无畸变，正常透视，人物空间比例真实 | 轻微镜头畸变，广角透视增强，前景略微放大，空间感突出 | 强烈镜头畸变，近处元素明显拉伸，沉浸式视觉冲击 |
| 景深 | 深景深，前后景全清晰，完整展现场景空间 | 中等景深，主体从环境分离，电影层次 | 浅景深，背景柔和虚化，视觉集中在人物情绪 |
| 散景 | 普通散景，背景灯光自然圆形虚化，标准电影镜头 | 轻微变形宽银幕散景，灯光横向拉伸，独特电影特征 | 强烈变形宽银幕散景，高光水平光晕，科幻大片质感 |

**英文关键词（喂英文工具直接取）**：

| 维度 | 无 | 轻微 | 强烈 |
|---|---|---|---|
| 色差 | no chromatic aberration, clean edges | slight chromatic aberration, subtle red-blue fringing | strong chromatic aberration, heavy color fringing, retro film look |
| 运动模糊 | static sharp, high shutter freeze | slight motion blur, natural motion trails | heavy motion blur, streaking, high-speed action |
| 畸变 | no lens distortion, natural perspective | slight wide-angle distortion, spatial push | strong barrel distortion, immersive perspective |
| 景深 | deep depth of field, everything sharp | medium depth of field, subject separation | shallow depth of field, creamy background blur |
| 散景 | natural round bokeh | slight anamorphic bokeh, horizontally stretched | strong anamorphic bokeh, horizontal flares, sci-fi epic |

> 用法：选中档位的中文写进正文、英文垫进技术段。情绪导向——浅景深+轻微色差=文艺叙事；强运动模糊+强畸变=动作冲击；强散景=科幻大片。
...
> ⚠️ **本节仅作为 VID-1.4 视频正式结构使用（V5.31 隔离·路由见〇章规则 6）**：精细镜头 / 正式交付的视频镜头按本结构逐项输出。**图片正式交付不得调用本节，统一使用「〇、图片 9 步精装结构」**。比外部常见的 10 步更多「场景与元素 / 道具细节 / 动态细节 / 肤质焦点 / 情绪关键词」五维。
>
...
| 大唐盛世·长安街市 | 长安街市，红墙金瓦宫阙，飞檐翘角，灯笼高悬 | 唐制圆领袍/齐胸襦裙，簪花仕女 | 宫灯、团扇、车马 | 开市——万国来朝，夜市千灯 | 长安一片月，万户捣衣声 |
| 故宫雪景 | 红墙白雪，金色琉璃瓦覆雪，飞檐积雪 | 深色棉袍宫人/素衣女子 | 宫灯、油纸伞 | 落雪——宫门深锁，雪满阶前 | 白雪却嫌春色晚 |
| 戏台旦角 | 戏台聚光，珠翠头面闪烁，后台虚化 | 京剧旦角华美戏服，珠翠头面 | 折扇、水袖、锣鼓 | 亮相——锣鼓一响，粉墨登场 | 台上一分钟，台下十年功 |
| 宋代山水 | 远山近水留白构图，扁舟独钓，水墨淡彩 | 蓑衣渔翁/素袍文人 | 钓竿、酒葫芦、纸伞 | 独钓——孤舟蓑笠，独钓寒江 | 一蓑烟雨任平生 |
| 民国旗袍 | 老上海石库门弄堂，雨巷昏灯 | 旗袍女子，波浪发，珍珠耳坠 | 油纸伞、黄包车、霓虹灯牌 | 弄堂——夜雨敲窗，旗袍入巷 | 上海滩的风，吹过石库门 |
### 现代题材速查
...
## 十四、后期字幕文案清单（后期层·不入 Prompt 主体·与「14 步详装结构」无关，剪辑时叠加）
...
### ★ H3 唯一主规范（VID-1.4 当前有效·结构层·MiniMax 官方五模式）

> ★ **全文档 H3 规则唯一入口**：本节点为"当前有效主规范"；其他 H3 章节（V4.2.7 八层 / V4.2.8 5 秒模板）已标注历史归档，仅供追溯，不得作为生产规则调用。内容层清单（写什么）与时长自适应节拍模板见本节点末尾。
...
**内容层清单（VID-1.4·写什么·吸收 V4.2.7 八层独有内容，旧章已归档）**：

1. **主动作**：只允许 1 个主动作
2. **微动作**：眼神 / 呼吸 / 手腕 / 衣袖 / 发丝 / 身体重心（防死静，1-2s 给一次微动态）
3. **道具动态**：怎么移动 / 移动方向 / 与手的接触关系 / 不突然消失或生成
4. **环境动态**：烛火 / 灯笼 / 雾 / 雨 / 雪 / 风 / 水面 / 纱帘 / 植物
5. **镜头运动**：一镜只选 1 个主要运镜（固定 / 缓慢推进 / 缓慢拉远 / 横向移动 / 轻微跟拍 / 轻微环绕）
6. **结束落点**：动作完成 / 动作停顿 / 视线落点 / 镜头停稳
7. **连续性禁止**：保持脸/发型/服装/道具/场景/光源/色调稳定；禁止换装/换脸/换场景/改变年龄

> 结构层（三字段正文）决定"怎么排版"，内容层决定"写什么"；两层合并 = H3 唯一完整规范。

**时长自适应节拍模板（VID-1.4·替换 V4.2.8 5 秒工业模板·H3 4-30s 通用）**：

```
0–15%   状态建立（人物保持首帧状态/微呼吸/极轻微眼神/环境自然运动）
15–60%  主动作（唯一主动作展开·含微动作与道具动态）
60–85%  辅助动态/反应（环境响应/第二层动态/情绪反应）
85–100% 落点（动作完成或停顿/视线落点/镜头停稳·收势帧衔接下一镜）
```

速算表（秒数 = 总时长 × 节拍百分比）：

| 总时长 | 状态建立 | 主动作 | 辅助动态 | 落点 |
|---|---|---|---|---|
| 4s | 0.6s | 1.8s | 1.0s | 0.6s |
| 5s | 0.75s | 2.25s | 1.25s | 0.75s |
| 8s | 1.2s | 3.6s | 2.0s | 1.2s |
| 10s | 1.5s | 4.5s | 2.5s | 1.5s |
| 15s | 2.25s | 6.75s | 3.75s | 2.25s |
| 30s | 4.5s | 13.5s | 7.5s | 4.5s |

> 规则：节拍百分比固定，秒数按总时长换算；每节拍段运镜速度/方向与节拍联动（主动作段运镜最稳）；落点段 = 收势帧（衔接下一镜，呼应"第三张不是死物件"收尾规则）。**5s 只是本模板的一个特例，不再是底层逻辑。**
...
### 5 秒镜头拆分策略（仅工具限 5s 时使用·VID-1.4 标注：非底层模板，5s 是时长自适应节拍的特例）
...
| 通用兜底 | 低质量, 模糊, 水印, 噪点, 过曝, 欠曝, 饱和度溢出, 锐化过度, 变形, 平均光, 棚拍光, 平光无层次 |
...
> 用法：PM-1.3「六、视频 QC」通过后，按本表定级归档。想快速调出：搜「P0」「筛选」。
...
**案例锚点净化状态（V5.32 新增·CASE/成品示范统一字段）**：

> 规则 9 对案例同样生效（V5.31 起）。每个案例/成品示范须标注净化状态：
> - 【净化状态 PASS】＝ 风格锚点 ≤2，可直接调用
> - 【净化状态 NEED-CLEAN】＝ 锚点超限，**调用前必须先净化**：语义压缩至 ≤2 个风格锚点，不改变画面效果；摄影参数（焦段/光圈/ISO）与真实材质（绢帛/胶片/琉璃等）另归技术字段，不计入风格锚点
> 净化示例：`奶油散景 + 胶片颗粒 + 低反差柔光 + 电影化` → `低反差柔光 + 电影化`（奶油散景/胶片颗粒并入景深与质感技术字段）
> 存量案例如未标注净化状态，调用前按本规则自查锚点数；超限一律先净化再投喂。

**5s 时间轴特例规则（V5.32 新增）**：

> 含 `0-1s / 1-3s / 3-4s / 4-5s` 时间轴的 CASE ③「5 秒动态版」与"5 秒动态"条目 = **【历史兼容案例 / 5s 特例】**，不是 H3 默认模板。调用时按 VID-1.4 时长自适应节拍模板换算（0-15% 状态建立 / 15-60% 主动作 / 60-85% 辅助动态 / 85-100% 落点），每段秒数 = 总时长 × 百分比；仅当目标时长恰为 5s 时可原样使用，其余时长一律换算。
...
- 血符：咬破指尖，以血在虚空画符，每一划都留下赤色光痕，符成后向目标一送。

> **法器手势写作公式**：掐诀位（拇指掐第几节/剑指/掌心托）+ 法器响应（泛光/嗡鸣/浮起/出鞘）+ 能量外放（指向目标/扩散成阵/冲天而起）+ 环境反馈（气流/光痕/尘土/衣袂）。

**A｜日常劳作动作 25～30（V5.25 入库·田园/市井/生活题材用）**

> 用法：日常动作写「环节拆解」——把一个劳作拆成 2-3 个连续小动作（如浣衣：浸帕→抹皂→搓洗→漂清），比笼统写"在洗衣服"更出画面。搜「劳作」「日常」「浣衣」调出。

A25｜浣衣：卷起袖口，半蹲水边，丝帕按入清水浸透（气泡上浮），抹皂角后细揉慢搓，再拖入河水漂洗至无沫，拧干抖开。
A26｜纺线：麻团置于膝上压住一端，一手匀速摇动纺车大轮（木轮吱呀转动），另一手从麻絮中均匀抽引麻线，纱锭旋转将线捻合拉直。
A27｜捣练：双手持木杵，围槽站立，木杵有节奏地上下捣击槽中丝料，水花随杵起落溅开。
A28｜推磨：一手扶磨柄匀速推转石磨，另一手一勺豆一勺水倒入磨眼，乳白豆浆沿磨缝缓缓流出。
A29｜劈柴：挑一段木段立地，双手握紧斧柄举过头顶，腰背发力，斧刃垂直劈下，木段应声裂开。
A30｜挑水：肩挑扁担，两手扶稳木桶，步伐随水桶晃动轻微调整，桶中水面荡开细纹，走到井边弯腰放下，提桶倒水。

**A｜舞蹈动作 31～36（V5.25 入库·水袖/剑舞/飞天/软舞·古风乐舞题材用）**

> 用法：舞蹈动作写「发力点→轨迹→收势」三节（如抛袖：肩部发力→袖如流水抛出划弧→腕转挽袖收回），配合旋转/折腰/腾跃；写舞必带神态（眉眼传情、目光随袖）。搜「舞蹈」「水袖」「剑舞」调出。

A31｜水袖抛挽：双臂自肩部发力，水袖如流水抛出、在空中划出完美弧线，收回时手腕轻转挽袖，袖尾掠过身侧，目光随袖尖移动。
A32｜水袖回旋：以足尖为轴旋转，长袖随身体荡开成圆，袖缘如水波翻涌，旋转中发丝与披帛同步飘起。
A33｜剑舞起势：起手式一个漂亮的挽花，剑光如银蛇吐信划破空气，腕转剑随，剑穗在空中绕成圆。
A34｜剑舞劈撩：劈剑如流星赶月、一往无前，撩剑如白鹤亮翅、优雅藏锋，收剑如灵蛇入洞；身法轻盈，腾挪跳跃间衣袂翻飞。
A35｜折腰软舞：腰肢向后折如柳枝，双臂自两侧伸展，指尖流苏垂落，裙摆随动作荡开成花，仰面时目光如水。
A36｜飞天舞姿：双臂伸展如翼、长巾飞扬，身姿翩然轻盈，足尖点地旋转，彩带与裙裾在空中画圆，如凌空而舞。

**舞蹈种类库（V5.27 入库·舞种点题用·配 A31-A36 动作使用）**

> 用法：写舞先点舞种（自带时代与气质），再套 A31-A36 动作；舞种名=画面气质标签。搜「舞蹈」「惊鸿」「胡旋」「盘鼓」调出。

| 舞种 | 时代/源流 | 气质与动作要点 |
|---|---|---|
| 惊鸿舞 | 唐·梅妃 | 写意模拟鸿雁翱翔，轻盈飘逸柔美空灵；双臂如翼展收，身姿起伏若飞鸟掠空，含蓄雅致 |
| 霓裳羽衣舞 | 唐·杨玉环 | 身披霞帔羽衣，舞步飘然回转，旋转时衣袂纷飞如月宫仙子，柔婉处如弱柳扶风 |
| 胡旋舞 | 唐·西域传入 | 急速旋转为核，左旋右转如回雪飘飖、转蓬之舞；弦鼓一声双袖举，奔车轮缓旋风迟，明快奔放 |
| 胡腾舞 | 唐·西域传入 | 矫捷明快活泼俏丽，腾跳为主，脚下踏地有声，动作刚健利落 |
| 柘枝舞 | 唐·西域传入 | 明快俏丽，鼓点伴奏，扭腰抖胯、眼神流转，装饰性动作点缀 |
| 盘鼓舞 | 汉·七盘舞 | 踏盘踏鼓腾跃起舞，轻重脚步敲击鼓点成节，融合杂技与舞蹈，刚捷轻盈，力量与柔美结合 |
| 绿腰舞 | 唐·软舞 | 长袖舞衣、长裙裾，舞姿轻盈疾徐变化；舞袖低回如莲花出浪，舞袖飞举如风搅雪花 |
| 剑器舞（剑舞） | 唐宋 | 执短剑而舞，甩动旋转短剑发出规律声响，战斗气氛，矫若游龙，缓急相间 |
| 长袖舞 | 汉·起源 | 长袖为器，抛甩翻飞，袖如流水，力与美兼收 |
| 折腰舞 | 六朝 | 腰肢为核，折腰旋体，体态轻盈，如柳临风 |
...
| 轻蔑 | 一侧嘴角快速上扬即收，下巴微抬，目光自上而下扫过，眉毛单侧轻挑 |
| 释然 | 眉头缓缓展开，唇间轻轻呼出一口气，肩膀随呼气下沉，眼神望向远处 |
| 犹豫 | 眼神在两点间来回移动，手指轻敲或摩挲，嘴唇抿起又松开，呼吸顿一拍 |
| 震惊/心碎 | 眼神瞬间失焦，呼吸停滞，嘴唇微张却发不出声，手停在半空，身体僵住 |

> **通用神态公式**：情绪 → 部位（眉/眼/唇/手/肩/呼吸任选 2-3 处）→ 微动态（收紧/泛红/下垂/上扬）→ 时间点（遇事瞬间/三秒后/缓过神后）。
...
### CASE 21-22：H3 整条视频实战范本（VID-1.4 实战范本·原 V5.11 入库·仙宫巨物观实测·不是教程是你自己跑出来的）
